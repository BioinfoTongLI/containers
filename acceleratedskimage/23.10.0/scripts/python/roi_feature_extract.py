...
def extract(
    ...
    df = pd.concat(peaks_in_all_tsls0)
...
def extract_2d(
    ...
    try:
        raw = AICSImage(raw_p)
        spacing = (raw.physical_pixel_sizes.Y, raw.physical_pixel_sizes.X)
    except:
        print("User didn't provide raw image, using label image as raw image")
        raw = lab
    print(lab.dims)
    ...
    for i, scene in enumerate(lab.scenes):
        ...
        props_table = regionprops_table(
            xp.array(lab_stack),
            intensity_image=xp.array(raw.get_image_dask_data("YXC", S=s, T=i).squeeze()),
        ...
        for col in df.columns:
            if col == "centroid-0":
                df["centroid-0_in_pixel"] = df[col] / spacing[0]
            elif col == "centroid-1":
                df["centroid-1_in_pixel"] = df[col] / spacing[1]
            elif col == "area":
                df["area_in_pixel2"] = df[col] / spacing[0] / spacing[1]
            else:
                pass
        props_of_scene[scene] = df
